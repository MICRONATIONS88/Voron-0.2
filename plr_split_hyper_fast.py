...
def process_gcode_hyper_fast(gcode_path, target_z):
    last_fan = None
    last_feedrate = None
    split_line_idx = None
    prev_z = 0.0
    ...
    with open(gcode_path, 'r', errors='replace') as f:
        for i, line in enumerate(f):
            ...
            # Moves
            if su.startswith('G0') or su.startswith('G1') or su.startswith('G2') or su.startswith('G3'):
                # Feedrate check
                f_idx = su.find('F')
                if f_idx != -1 and (f_idx == 0 or su[f_idx-1].isspace()):
                    digits = []
                    for char in su[f_idx+1:]:
                ...
                # Z height check
                z_idx = su.find('Z')
                if z_idx != -1 and (z_idx == 0 or su[z_idx-1].isspace()):
                    digits = []
                    for char in su[z_idx+1:]:
...
def main():
    ...
    split_index, last_fan, last_feedrate = process_gcode_hyper_fast(gcode_path, resume_z)
